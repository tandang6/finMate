...
from logic_alerts import get_logic_alerts       # 실제 시장 데이터 기반 로직 알림
...
class LogicAlertResponse(BaseModel):
    """대시보드 로직 알림 카드 응답"""
    id: str
    kind: str
    title: str
...
# ------------------------------------------------------------------------------
# [3-c-2] 실제 데이터 기반 로직 알림 API (/api/logic-alerts)
# ------------------------------------------------------------------------------
@app.get("/api/logic-alerts", response_model=List[LogicAlertResponse])
def logic_alerts():
    return [LogicAlertResponse(**alert) for alert in get_logic_alerts()]
